...
func (s *DockerSuite) TestRunVolumesFromInReadWriteMode(c *check.C) {
	...
	if out, _, err := dockerCmdWithError("run", "--volumes-from", "parent:bar", "busybox", "touch", fileInVol); err == nil || !strings.Contains(out, "invalid mode for volumes-from: bar") {
		c.Fatalf("running --volumes-from fparent:bar should have failed with invalid mount mode: %q", out)
	}
...
func (s *DockerSuite) TestRunVolumesFromSymlinkPath(c *check.C) {
	// FIXME Windows: In TP4 builds, using mklink inside a container image
	// using docker build causes docker run to subsequently fail with an
	// error from CreateSandboxLayer.
	testRequires(c, DaemonIsLinux)
...
func (s *DockerSuite) TestRunAllowBindMountingRoot(c *check.C) {
	...
	dockerCmd(c, "run", "-v", mount, "busybox", "ls", targetDir)
}
